...
avg_margin = df_cons_summary["margin_percent"].mean()
avg_enp = df_cons_summary["ENP"].mean()
most_competitive = df_cons_summary.sort_values("margin_percent").iloc[0]
least_competitive = df_cons_summary.sort_values("margin_percent", ascending=False).iloc[0]

st.markdown(f"""
### 🔎 National Overview

- ค่าเฉลี่ยส่วนต่างคะแนนทั่วประเทศอยู่ที่ **{avg_margin:.2f}%**
- ค่าเฉลี่ย Effective Number of Parties (ENP) อยู่ที่ **{avg_enp:.2f}**

### 🔥 เขตที่สูสีที่สุด
- เขต: **{most_competitive['cons_id']}**
- จังหวัด: **{most_competitive['province']}**
- Margin: **{most_competitive['margin_percent']:.2f}%**

### 🏆 เขตที่ชนะขาดที่สุด
- เขต: **{least_competitive['cons_id']}**
- จังหวัด: **{least_competitive['province']}**
- Margin: **{least_competitive['margin_percent']:.2f}%**
""")
